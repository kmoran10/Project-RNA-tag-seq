...
### initial cleaning and checking raw counts of genes per subject and ROI. 
...
write.csv(bcx1, "raw_data/rnaseq_counts_ms.csv", row.names = F)
...
ggsave("imgs/braincounts_bygroup.png",counts2, height = 5,width= 5, dpi = 150)
...
ggsave("imgs/braincounts_bygroupandROI.png",counts3, height = 5,width= 5, dpi = 150)
...
write.csv(ARC_id, "ms_brain_data/ARC_id.csv", row.names = T)
write.csv(DMH_id, "ms_brain_data/DMH_id.csv", row.names = T)
write.csv(LH_id, "ms_brain_data/LH_id.csv", row.names = T)

write.csv(idxx, "ms_brain_data/all_id.csv", row.names = T)
...
write.csv(ARC_counts, "ms_brain_data/ARC_counts.csv", row.names = T)
write.csv(DMH_counts, "ms_brain_data/DMH_counts.csv", row.names = T)
write.csv(LH_counts, "ms_brain_data/LH_counts.csv", row.names = T)

write.csv(bcx1, "ms_brain_data/all_counts.csv", row.names = T)
