...
coldata <- read.csv("ham_brain_data/DMH_id.csv", row.names = "X")
coldata <- coldata[-1,]
...
saveRDS(v.dl, "results/limma_vdl_DMH.RDS")
...
saveRDS(q.dl,("results/limma_vdl_cutoff5_2000_tworand_DMH.RDS"))

####

q.dl <- readRDS("results/limma_vdl_cutoff5_2000_tworand_DMH.RDS")
...
DMH_limma_results <- topTable(tmp1, sort.by = "P", n = Inf) %>% 
  rownames_to_column('symbol') %>% 
  dplyr::select(symbol,logFC,P.Value,adj.P.Val)



DMH_limma_results1 <- DMH_limma_results %>% 
...
saveRDS(DMH_limma_results1,"results/DMH_limma_results.RDS")
