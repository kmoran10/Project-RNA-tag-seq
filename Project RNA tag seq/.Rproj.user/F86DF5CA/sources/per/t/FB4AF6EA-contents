...
## following the DESeq2 guide for my LH data
...
# t.test(wt.day.15 ~ group, data = coldata)
# t.test(wt.rate.pct ~ group, data = coldata)
# t.test(wt.rate.g ~ group, data = coldata)
# t.test(food.total ~ group, data = coldata)
# t.test(total.fat ~ group, data = coldata)
# t.test(g.hoarded ~ group, data = coldata)
...
write.csv(as.data.frame(resOrdered), 
          file="results/LH_bygroup_DESeq_results.csv")
...
ggplot(pcaData, aes(PC1, PC2, color=group, label = rownames(pcaData))) +
  ...
  coord_fixed() + geom_text() + labs(title="LH")
